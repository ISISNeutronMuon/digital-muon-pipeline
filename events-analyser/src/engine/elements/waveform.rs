...
impl FlattenableWithIndex for Waveform {
    ...
    fn flatten(&self, arrays: &[Array], index: usize) -> Result<FlatWaveform, Self::Error> {
        match self {
            Waveform::Flat { width } => Ok(FlatWaveform::Flat { width: width.flatten(arrays, index)? }),
            Waveform::Triangular { base_width } => Ok(FlatWaveform::Triangular { base_width: base_width.flatten(arrays, index)? }),
            Waveform::Gaussian { sd} => Ok(FlatWaveform::Gaussian { sd: sd.flatten(arrays, index)? }),
        }
    }
}
