...
use crate::engine::{Array, FlattenableWithIndex, utils::{Function, Interval}};

pub(crate) trait Number : Add<Self, Output=Self> + Mul<Self, Output=Self> + Copy + Clone + NumCast {}

impl<T> Number for T where T : Add<Self, Output=Self> + Mul<Self, Output=Self> + Copy + Clone + NumCast {}
...
impl<T: Number + PartialEq + PartialOrd> ConstantFilter<T> {
    pub(crate) fn is_valid(&self, other_value: T) -> bool {
        match self {
            ...
            ConstantFilter::AnyInRange(interval) => interval.range_inclusive().contains(&other_value),
...
impl<T : Number> FlattenableWithIndex for ValueFilter<T> {
    ...
    fn flatten(&self, arrays: &[Array], index: usize) -> Result<ConstantFilter<T>, Self::Error> {
        match self {
            ValueFilter::Dependant(dependancy) => Ok(ConstantFilter::Is(
                match dependancy {
...
impl<T : Number> FlattenableWithIndex for Value<T> {
    ...
    fn flatten(&self, arrays: &[Array], index: usize) -> Result<T, Self::Error> {
        match self {
            Value::Dependant(dependancy) => 
                Ok(match dependancy {
                    Dependancy::Array(array) => T::from(arrays.iter()
                        .find(|a|a.has_name(&array))
                        .ok_or_else(||format!("Cannot find {array} in list of arrays."))?
                        .get_element(index)
                    ).ok_or_else(||format!("Cannot convert array element to correct type."))?,
                    Dependancy::Function(function) => function.apply(
                        T::from(index).ok_or_else(||format!("Cannot convert array element to correct type."))?
                    ),
                }),
