...
use crate::{analysis::metrics::{MetricAggregatedResult, MetricChannelResult, SumWithSumOfSqrs}, engine::{FlatAlgorithm, FlatMetricFalseCount, FlatWaveform, Metric}, event::ChannelData};
...
impl MetricChannelResult for FalseCount {
    ...
    fn push(&mut self, waveform: &FlatWaveform, algorithm: &FlatAlgorithm, by_topic : &[ChannelData]) {
        self.positive_sum.add_to(get_false_positives(self.true_topic, self.estimate_topic, waveform, algorithm, by_topic));
        self.negative_sum.add_to(get_false_positives(self.true_topic, self.estimate_topic, waveform, algorithm, by_topic));
    }
}
...
impl MetricAggregatedResult for CompletedFalseCount {
    ...
    fn aggregate(source: &HashMap<Channel, Self::Channel>) -> Self {
        let (sum_of_means, sum_of_sds) = source
            .iter()
            .map(|(_, count)|count.positive_sum.mean_and_stddev(count.num as f64))
            .fold(Default::default(),
                |(acc_mean, acc_sd) : (f64,f64), (mean, sd)|
        ...
        let (sum_of_means, sum_of_sds) = source
            .iter()
            .map(|(_, count)|count.negative_sum.mean_and_stddev(count.num as f64))
            .fold(Default::default(),
                |(acc_mean, acc_sd) : (f64,f64), (mean, sd)|
                    (acc_mean + mean, acc_sd + sd)
            );
...
fn group_data_by<F>(filter: F, data_to_group_by: &ChannelData, data_to_group: &ChannelData) -> (Vec<Vec<usize>>, Vec<usize>)
    where F : Fn(&ChannelData, usize, Time, Intensity) -> bool {
    ...
    for (index, (time,intensity)) in data_to_group_by.get_time_intensity().iter().enumerate(){
        match left_bound.peek() {
            ...
            Some(Some(left_bound_index)) => {
                let index = data_to_group.find_nearest_in_time_after_index(*left_bound_index, *time);
...
pub(crate) fn sort_estimates_by_true(true_topic_index: usize, estimate_topic_index: usize, waveform: &FlatWaveform, algorithm: &FlatAlgorithm, collection_by_topic: &[ChannelData]) ->  (Vec<Vec<usize>>, Vec<usize>) {
...
pub(crate) fn get_false_positives(true_topic_index: usize, estimate_topic_index: usize, waveform: &FlatWaveform, algorithm: &FlatAlgorithm, collection_by_topic: &[ChannelData]) -> f64 {
    let true_data_bucket = sort_estimates_by_true(true_topic_index, estimate_topic_index, waveform, algorithm, collection_by_topic);
    0.0
}